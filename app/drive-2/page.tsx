...
function calculateWheelAngles(wheelLocations : Vector2D[], steeringAngle: number, angle: number) {
    ...
    if(!isFinite(radius)) return wheelLocations.map(() => -angle);
    const turningCircleCenter = Vector.polar(radius, Math.PI/2 - angle);
    const directions = wheelLocations.map((loc) => Vector.sub(loc, turningCircleCenter));
    

    const angles = directions.map((dir) => {
        if(steeringAngle > 0) return Vector.angle(dir) + Math.PI/2;
        return Vector.angle(dir) - Math.PI/2;
    });
    return angles;
}
...
function calculateWheelSpeeds(wheelLocations : Vector2D[], speed: number, radius: number, angle: number) {
    ...
    return directions.map((dir) => speed * Vector.mag(dir) / imaginaryWheelDistance)
}
...
function drawWheels(ctx : CanvasRenderingContext2D, wheelLocations: Vector2D[], wheelAngles: number[], map: (pos: Vector2D) => Vector2D) {    
    ...
    wheelLocations.forEach((point, i) => {
        ctx.beginPath();
        const direction = { x: 40 * Math.sin(wheelAngles[i]), y: 40* Math.cos(wheelAngles[i])}
        const mid = map(point);
    ...
    if(ctx != null) {
        ...
        requestAnimationFrame(() => {
            ...
            wheelLocations.forEach((loc) => {
                const mapped = map(loc);
                ctx.moveTo(turningCircleCenterMapped.x, turningCircleCenterMapped.y);
...

            {
                ...
                wheelLocations.forEach((loc) => {
                    drawDirectionArc(ctx, turningCircleCenterMapped, map(loc), 10, direction, size);
                });
